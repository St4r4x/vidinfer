...
Exit codes: 0 success, 2 invalid input, 1 unexpected error.
"""
...
class InputError(Exception):
    """Invalid input (exit code 2): retrying will not help, the input must be fixed."""


def kv(event: str, **fields: object) -> str:
    """One logfmt line: readable by a human, parsable with shlex.split + split('=', 1)."""
    parts = [f"event={event}"]
...
def setup_logging(path: Path) -> None:
    logging.basicConfig(
        ...
        force=True,  # re-entrant: tests call main() several times
    )
...
def parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    p.add_argument("--fps", type=int, default=10, help="inferences per second of VIDEO time (default 10)")
    p.add_argument("--model", default="yolo26s.pt", help=f"pinned weights ({', '.join(model_lib.WEIGHTS)}) or a .pt")
    p.add_argument("--device", choices=["auto", "cuda", "cpu"], default="auto")
    p.add_argument("--max-frames", type=int, help="stop after N inferences (smoke test)")
...
def run(args: argparse.Namespace) -> int:
    ...
    expected = int(Fraction(meta["duration_s"]).limit_denominator(10**6) * args.fps)
    log.info(
        kv(
            "sampling",
            target_fps=args.fps,
        ...
        log.info(
            kv(
                ...
                **{f"yolo_{k}_ms": v for k, v in yolo.items()},
                cpu_util=(cpu_now - cpu_block) / wall,
    ...
    if not stopped_early and n != expected:
        log.warning(kv("count_mismatch", n_inferences=n, expected=expected))
    log.info(
        kv(
            "timing",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        log.exception(kv("unexpected_error"))
        return 1
